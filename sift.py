...
def ocr(test_img, characters):
    ...
    labelled_img = cv2.imread('ccl.jpg',cv2.IMREAD_GRAYSCALE)
    res = recognition(labelled_img,ccd)
    return res
...
def detection(image):
    ...
    temp = np.zeros((labelled_img.shape[0],labelled_img.shape[1]))
...
def recognition(image,ccd):
    ...
    for key in ccd:
        ...
        # bbox_image=cv2.resize(bbox_image, None, fx= 1.5, fy= 1.5, interpolation= cv2.INTER_LINEAR)
        kp, des = sift.detectAndCompute(bbox_image,None)
        if des is None:
            # x = {
            #     "bbox":current_bbox,
